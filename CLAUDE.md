...
### 핵심 강점: 섹션별 조합
- 다른 청첩장 서비스: 템플릿 하나를 통째로 선택
- Maison de Letter: 갤러리, 캘린더, 인사말 등 **섹션별로 골라서 조합**
- "이 업체 갤러리는 예쁜데 캘린더가 별로" → 두 업체의 장점만 조합 가능

### 추가 강점: 프리셋 요청
- 원하는 디자인이 없으면 요청하면 만들어드림
- "원하는 게 없으면 만들어드려요"

### 3초 법칙
- 히어로 섹션에서 3초 안에 핵심 가치 전달
- 조합 공식 애니메이션으로 시각화: [갤러리] + [캘린더] + [인사말] = 내 청첩장
