...
### 2026-01-02: SubwayBuilder 랜딩 페이지로 전환
- **이유**: "AI가 만들어준다"에서 "사용자가 직접 선택한다"로 핵심 가치 전환
- **변경**:
  - 캐치프레이즈: "AI가 바로 시안을 만듭니다" → "당신만의 이야기를 담은 청첩장"
  - 새 플로우: 템플릿 선택 → 섹션 프리셋 선택 → 에디터에서 완성
  - 4개 섹션 프리셋 시스템: 인사말, 예식일시, 갤러리, 오시는길
  - 에디터에서 추가 블록 삽입 가능
- **파일**:
  - `src/components/landing/SubwayBuilderLanding.tsx`
  - `src/components/landing/subway/` (Context, HeroSelector, SectionRow 등)
