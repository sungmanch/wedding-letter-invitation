...
### 1. 블록 레벨: Auto Layout으로 전환

**Hero 블록을 제외한 모든 블록의 `layout.mode`는 `'auto'`로 설정합니다.**

| 블록 타입 | layout.mode | 이유 |
|----------|-------------|------|
| `hero` | **absolute** | 배경 이미지 중심, 정밀 배치 필요 |
| 그 외 모든 블록 | **auto** | 텍스트 오버플로우 방지, 콘텐츠 가변 대응 |

### 2. 요소 레벨: 디자인 요소는 Absolute 유지

**블록 내부의 장식/디자인 요소는 `layoutMode: 'absolute'`를 유지합니다.**

| 요소 역할 | layoutMode | 예시 |
|----------|-----------|------|
| **콘텐츠** | (없음 = auto) | 텍스트, 버튼, 주요 이미지 |
| **장식** | `'absolute'` | sparkle, flower, 배경 장식, 아이콘 |

```
┌─────────────────────────────────────┐
│ Block (layout.mode: 'auto')         │
│                                     │
│  ┌─ 장식 (absolute) ─┐              │
│  │   sparkle.png     │              │
│  └───────────────────┘              │
...
**핵심: 블록은 auto, 장식 요소만 absolute 유지**
