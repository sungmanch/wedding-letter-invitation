# Maison de Letter UX Design Specification
...
**프로젝트:** 모바일 청첩장 - 당신만의 이야기를 담은 청첩장

**비전:** 랜딩에서 템플릿과 섹션 프리셋을 미리 선택하고, 에디터에서 상세 정보를 입력하여 완성하는 커스터마이징 청첩장 서비스. 사용자는 결과물을 미리 보면서 선택하고, 에디터에서 추가 블록을 삽입할 수 있다.
...
**핵심 가치:**
1. **미리보기 선택**: 랜딩에서 템플릿+섹션을 결과물 보면서 선택
2. **6가지 프리미엄 템플릿**: Classic, Casual, Minimal, Modern, Bright, Mono
3. **4개 섹션 프리셋**: 인사말, 예식일시, 갤러리, 오시는길
4. **에디터에서 완성**: 상세 정보 입력 + 추가 블록 삽입
5. **실시간 편집**: 모든 요소 클릭 수정
...
| 기존 서비스 | 우리 서비스 |
|------------|------------|
| 완성된 템플릿 선택 | 랜딩에서 템플릿+섹션 미리보며 선택 |
| 폼 입력 후 미리보기 | 실시간 2패널 편집 |
| 고정된 섹션 구성 | 4개 섹션 프리셋 + 추가 블록 삽입 |
| 전문가만 수정 가능 | 모든 요소 클릭하여 직접 수정 |
...
```
[1. 랜딩 - 템플릿 선택] → [2. 섹션 프리셋 선택] → [3. 생성하기]
                                                       ↓
[4. 회원가입/로그인] → [5. 에디터 진입 (선택 반영)] → [6. 정보 입력 + 블록 추가]
                                                       ↓
[9. 공유] ← [8. 결제] ← [7. 미리보기]
```

**Step 1-2. 랜딩에서 선택 (핵심 차별점):**
- 6가지 프리미엄 템플릿 중 선택
- 4개 핵심 섹션(인사말, 예식일시, 갤러리, 오시는길)의 프리셋 선택
- 미리보기 이미지를 보면서 선택
