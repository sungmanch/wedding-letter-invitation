...
개인화된 모바일 청첩장 생성 서비스. 기존의 뻔한 템플릿 기반 청첩장 서비스와 달리, AI가 커플의 스토리와 선호도를 바탕으로 유니크한 청첩장 디자인을 생성해준다. 10개의 핵심 섹션 타입과 7개의 인트로 스타일 프리셋으로 완전히 맞춤화된 청첩장을 만들 수 있으며, 게스트들은 축하 메시지를 통해 특별한 순간을 함께 나눌 수 있다.
...
- accounts - 축의금 계좌
- contact - 연락처
- guestbook - 방명록 (게스트 메시지)
- music - 배경음악 (플로팅 FAB)

**5. 확장 섹션 (Post-MVP)**
- Video, Interview, Timeline, D-DAY 섹션 (컴포넌트 구현 완료, 시스템 미통합)
...
**확장 기능**
- 확장 섹션 시스템 통합 (Video, Interview, Timeline, D-DAY)
- 영상 청첩장 (전체 페이지 영상)
- RSVP 참석 확인 기능
...
- **FR45:** 🔜 Host는 Video 섹션에 영상을 추가할 수 있다 (컴포넌트 존재, 미통합)
- **FR46:** 🔜 Host는 Timeline 섹션으로 커플 스토리를 보여줄 수 있다 (컴포넌트 존재, 미통합)
- **FR47:** 🔜 Host는 Interview 섹션으로 웨딩 Q&A를 추가할 수 있다 (컴포넌트 존재, 미통합)
- **FR48:** 🔜 D-DAY 섹션은 결혼일까지 실시간 카운트다운을 표시한다 (컴포넌트 존재, 미통합)
...
- **FR20:** ✅ Host는 청첩장 결제(9,900원)를 완료할 수 있다 (Polar.sh)
...
| **구현 상태** | FR 65개 (✅ 49개 구현 / 🔜 16개 미구현) |
| **핵심 섹션** | 10개 (intro, greeting, date, venue, gallery, parents, accounts, contact, guestbook, music) |
