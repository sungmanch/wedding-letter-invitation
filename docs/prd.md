...
이 서비스는 다르다:
- **프롬프트 기반 AI 생성**: 느낌만 말하면 AI가 바로 청첩장을 생성 ("럭셔리하게 만들어줘")
- **레퍼런스 스타일 분석**: Pinterest/Instagram URL 또는 이미지로 스타일 참고 가능
...
**Host Flow (청첩장 생성):**
```
Landing (PromptHeroLanding)
→ 프롬프트 입력 (Typewriter 예시로 가이드)
   - "럭셔리하고 고급스럽게 만들어줘"
   - "사진이 예뻐서 사진 중심으로"
   - "심플하게, 정보만 깔끔하게"
→ (선택) 레퍼런스 이미지/URL 첨부
   - Pinterest, Instagram URL 붙여넣기
   - 이미지 파일 업로드/드래그
   - AI가 자동 분석 (mood, colors, typography, layout)
→ [생성하기] 클릭
→ 회원가입/로그인 (비로그인 시)
   - sessionStorage에 입력값 보존
   - 로그인 후 자동 복원
→ /api/landing/generate 호출
   - 문서 생성 + AI 디자인 생성 (Gemini)
→ /se2/[id]/edit 3탭 편집
   - 콘텐츠: 섹션별 정보 입력
   - 디자인: 글꼴, 색상, 배경
   - 공유: OG 메타데이터 설정
...
_This PRD captures the essence of Maison de Letter - 느낌만 말하면 AI가 만들어주는 나만의 특별한 청첩장_
