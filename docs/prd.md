...
1. **템플릿 선택**: 6가지 프리미엄 템플릿 중 원하는 스타일 선택
   - Classic(클래식), Casual(캐주얼), Minimal(미니멀)
   - Modern(다크), Bright(브라이트), Mono(모노크롬)

2. **섹션 프리셋 선택**: 4개 핵심 섹션의 스타일을 미리 선택
   - 인사말 (greeting-parents)
   - 예식일시 (calendar)
   - 갤러리 (gallery)
   - 오시는길 (location)

3. **에디터에서 완성**: 선택한 템플릿+섹션이 반영된 상태로 에디터 진입
   - 구체적인 정보 입력 (이름, 날짜, 장소 등)
   - 추가 블록 삽입 가능
   - 모든 요소를 클릭해서 바로 수정

**핵심 차별점:**
- **미리보기 선택**: 랜딩에서 결과물을 미리 보면서 선택
- **6가지 프리미엄 템플릿**: 전문 디자이너가 제작한 고품질 디자인
- **섹션별 프리셋**: 각 섹션마다 다양한 레이아웃 옵션
- **실시간 편집**: 에디터에서 모든 요소 클릭 수정 + 추가 블록 삽입
...
**1. 프롬프트 기반 청첩장 생성 플로우**
- **Landing에서 바로 시작**: 프롬프트 입력 → 로그인 → AI 생성 → Editor
- **Typewriter 예시**: "럭셔리하게 만들어줘" 같은 일상 언어로 가이드
- **레퍼런스 스타일 분석**: Pinterest/Instagram URL 또는 이미지 업로드로 스타일 참고
...
- **프롬프트 기반 생성:** 느낌만 말하면 AI가 바로 생성 (Typewriter로 예시 제공)
- **레퍼런스 스타일 참고:** URL/이미지로 원하는 스타일 전달
...
**Host Flow (청첩장 생성):**
```
Landing (SubwayBuilderLanding)
→ Step 1: Hero 템플릿 선택 (6가지 중)
   - Classic(클래식), Casual(캐주얼), Minimal(미니멀)
   - Modern(다크), Bright(브라이트), Mono(모노크롬)
→ Step 2: 섹션 프리셋 선택 (4개 섹션)
   - 인사말, 예식일시, 갤러리, 오시는길
   - 각 섹션별 다양한 레이아웃 프리셋 중 선택
→ [생성하기] 클릭
→ 회원가입/로그인 (비로그인 시)
   - sessionStorage에 선택값 보존
   - 로그인 후 자동 복원
→ /se2/[id]/edit 에디터 진입 (선택한 템플릿+섹션 반영)
   - 콘텐츠: 구체적인 정보 입력 (이름, 날짜, 장소 등)
   - 추가 블록 삽입 가능
   - 디자인: 색상, 폰트 조정
   - 공유: OG 메타데이터 설정
→ /se2/[id]/dashboard 관리
...
- **FR5:** ✅ Host는 Landing에서 프롬프트로 청첩장을 생성할 수 있다
- **FR5-1:** ✅ Typewriter로 프롬프트 예시를 보여주고 입력받는다 ("럭셔리하게 만들어줘")
- **FR5-2:** ✅ (선택) 레퍼런스 이미지/URL을 첨부하여 스타일을 참고할 수 있다
- **FR5-3:** ✅ 생성 버튼 클릭 시 AI가 청첩장을 만들고 Editor로 이동한다
- **FR5-4:** ✅ 비로그인 시 sessionStorage에 입력값을 보존하고 로그인 후 복원한다
...
- **FR12:** ✅ Host는 프롬프트로 원하는 분위기/스타일을 전달할 수 있다
- **FR13:** ✅ 시스템은 Google Gemini로 스타일 + 레이아웃을 생성한다
- **FR14:** ✅ Host는 생성된 디자인을 Editor에서 확인하고 수정할 수 있다
- **FR15:** ✅ Host는 디자인을 재생성할 수 있다 (무료)
- **FR36:** ✅ Host는 레퍼런스 이미지/URL을 첨부하여 AI가 분석하게 할 수 있다
- **FR37:** ✅ 시스템은 업로드된 이미지를 분석하여 무드/컬러/스타일을 추출한다 (Gemini Vision)
...
- **FR55:** ✅ Landing에서 레퍼런스 URL을 입력받는다 (800ms debounce 후 자동 분석)
- **FR56:** ✅ Landing에서 레퍼런스 이미지를 업로드받는다 (드래그앤드롭 지원)
- **FR57:** ✅ Gemini Vision으로 이미지를 분석하여 스타일 정보를 추출한다
- **FR58:** ✅ 분석 결과 (mood, colors, typography, layout, keywords)를 표시한다
- **FR59:** ✅ 분석 결과는 AI 생성 시 프롬프트에 포함된다
- **FR60:** ✅ 레퍼런스 없이 프롬프트만으로도 생성 가능하다
- **FR61:** ✅ 비로그인 시 입력값은 sessionStorage에 보존되고 로그인 후 복원된다
...
| **핵심 가치** | 프롬프트 + 레퍼런스로 만드는 나만의 개인화 청첩장 |
| **타겟** | 남들과 다른 청첩장을 원하는 20-30대 예비 신부 |
| **비즈니스 모델** | 건당 19,900원 |
| **MVP 성공 기준** | 1개월 내 20명 결제 |
| **구현 상태** | FR 70개 (✅ 55개 구현 / 🔜 15개 미구현) |
| **생성 플로우** | Landing (프롬프트 + 레퍼런스) → 로그인 → AI 생성 → Editor |
...
| **핵심 기술** | Google Gemini AI (Vision 포함), Design Token System, 30+ 폰트, 40+ 애니메이션 |
...
| 2025-12-22 | 5.0 | Landing 리디자인 반영: (1) Stage 1/2 → 프롬프트 기반 생성으로 변경, (2) 레퍼런스 스타일 분석 추가 (Gemini Vision), (3) Landing → 로그인 → Editor 플로우로 단순화, (4) Typewriter 프롬프트 예시, (5) FR5 시리즈 및 FR55-61 업데이트 | Claude |
| 2026-01-02 | 5.1 | SubwayBuilder 랜딩으로 전환: (1) 캐치프레이즈 변경 ("당신만의 이야기를 담은 청첩장"), (2) 랜딩에서 템플릿+4개 섹션 프리셋 선택 → 에디터에서 완성 플로우, (3) 핵심 가치를 "AI 생성"에서 "미리보기 선택 + 섹션별 커스터마이징"으로 전환, (4) Host Flow 업데이트 | Claude |

---

_This PRD captures the essence of Maison de Letter - 당신만의 이야기를 담은 청첩장_
