...
사용자 입력: "우아하고 모던한 청첩장"
           ...
           ▼
┌─────────────────────────────────────────────────────────┐
│  generateIntroOnly() - generation-service.ts:308       │
└─────────────────────────────────────────────────────────┘
           │
           ├────────────────────────────────────────────────┐
           ▼                                                ▼
...
│  🤖 AI 호출 #1              │          │  🤖 AI 호출 #2                      │
│  generateStyle()            │          │  selectVariants()                   │
│  gemini-provider.ts:91      │          │  gemini-provider.ts:127             │
│                             │          │                                     │
│  입력: prompt, mood         │          │  입력: prompt + variant 목록        │
│  출력: StyleSchema          │          │  출력: { variantId: "elegant" }     │
│  - 색상 팔레트              │          │  - 9개 중 1개 선택                  │
│  - 폰트 설정                │          │                                     │
│  - 테마 토큰                │          │  (generateIntroSection 내부에서 호출)│
└─────────────────────────────┘          └─────────────────────────────────────┘
           │                                                │
           └────────────────────┬───────────────────────────┘
                                ▼
...
| 호출 | 함수 | 파일:라인 | 목적 | temperature |
|------|------|-----------|------|-------------|
| **#1** | `generateStyle()` | gemini-provider.ts:91 | 색상/폰트/테마 생성 (StyleSchema) | 0.7 |
| **#2** | `selectVariants()` | gemini-provider.ts:127 | 9개 intro variant 중 1개 선택 | 0.5 |

**순수 함수 (AI 호출 없음)**:
- `resolveTokens()` - StyleSchema → SemanticDesignTokens
- `generateCssVariables()` - Tokens → CSS 변수 문자열
- `resolveSkeletonToScreen()` - variant + tokens → 렌더링 가능한 SectionScreen
