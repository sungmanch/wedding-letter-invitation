...
/**
 * Super Editor v2 - Branch Edit Client
 *
 * 브랜치 편집 화면
 * - blocks, style, animation만 수정 가능
 * - data는 parent에서 상속 (읽기 전용)
 */

import { useState, useCallback, useMemo, useRef, useEffect } from 'react'
import Link from 'next/link'
import type { BranchWithData } from '@/lib/super-editor-v2/actions/branch'
import type { EditorDocument, Block, Element, StyleSystem, WeddingData } from '@/lib/super-editor-v2/schema/types'
import { updateBranch as saveToServer, updateBranchOgMetadata } from '@/lib/super-editor-v2/actions/branch'
import { uploadImage } from '@/lib/super-editor-v2/actions/document'
...
import { BranchDataTab } from './BranchDataTab'
...
import { useBranchLocalStorage } from '@/lib/super-editor-v2/hooks/useBranchLocalStorage'
...
import type { ThemePresetId, GlobalAnimation } from '@/lib/super-editor-v2/schema/types'
...
interface BranchEditClientProps {
  branch: BranchWithData
}
...
export function BranchEditClient({ branch }: BranchEditClientProps) {
  // 초기 문서 변환 (브랜치 데이터 + parent data 결합)
  const initialDocument = useMemo((): EditorDocument => ({
    id: branch.id,
    version: 2,
    meta: {
      title: branch.title,
    ...
    blocks: branch.blocks as Block[],
    style: branch.style as StyleSystem,
    animation: (branch.animation ?? {}) as GlobalAnimation,
    data: branch.data, // parent에서 상속
  }), [branch])

  // 로컬 저장소 훅 - IndexedDB 기반 (브랜치용)
  const {
    document: editorDoc,
    updateDocument,
    save,
    ...
    lastSaved,
    discardChanges,
  } = useBranchLocalStorage({
    branchId: branch.id,
    initialDocument,
    parentData: branch.data, // parent data는 항상 최신으로 유지
    onSave: async (doc) => {
      await saveToServer(branch.id, {
        blocks: doc.blocks,
        style: doc.style,
  ...
  const [og, setOg] = useState<OgMetadata>(() => ({
    title: branch.ogTitle || '',
    description: branch.ogDescription || '',
    imageUrl: branch.ogImageUrl || null,
  }))
  ...
  // 이미지 업로드 (parent 문서 ID 사용 - 이미지는 공유)
  const handleUploadImage = useCallback(async (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader()
      reader.onload = async (event) => {
        const base64Data = event.target?.result as string
        // 브랜치는 parent의 storage 경로 사용
        const result = await uploadImage(branch.parentDocumentId, {
          data: base64Data,
          filename: file.name,
          mimeType: file.type,
        })
  ...
  }, [branch.parentDocumentId])

  // OG 업데이트 (즉시 서버 저장)
  const handleOgChange = useCallback(async (newOg: OgMetadata) => {
    setOg(newOg)
    try {
      await updateBranchOgMetadata(branch.id, {
        ogTitle: newOg.title,
    ...
    <div className="h-screen flex flex-col bg-[var(--ivory-100)] text-[var(--text-primary)]">
      ...
      <header className="flex-shrink-0 h-14 border-b border-[var(--sand-100)] bg-[var(--ivory-100)]/95 backdrop-blur-sm flex items-center justify-between px-4">
        <div className="flex items-center gap-4">
          <Link
            href={`/se2/${branch.parentDocumentId}/edit`}
            className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors"
          >
            ← 원본 문서로
          </Link>
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded text-xs bg-[var(--sage-100)] text-[var(--sage-700)]">
              브랜치
            </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {!branch.isPaid && (
            <a
              href={`https://buy.polar.sh/polar_cl_branch_product?metadata=${encodeURIComponent(JSON.stringify({ branchId: branch.id }))}`}
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === 'content' && (
              <ContentTab
                ...
                onDataChange={() => {}} // 브랜치에서는 data 변경 불가
            ...
            {activeTab === 'data' && (
              <BranchDataTab
                data={editorDoc.data}
                parentDocumentId={branch.parentDocumentId}
                parentTitle={branch.parentTitle}
              />
            )}
            {activeTab === 'design' && (
              <DesignTab
            ...
            {activeTab === 'share' && (
              <ShareTab
                ...
                shareUrl={branch.status === 'published' ? `/share/branch/${branch.id}` : null}
                isBranch
              />
            )}
          </div>
